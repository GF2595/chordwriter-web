...
import { get, isArray, noop, set, isNil } from 'lodash';
import React, {
    createContext,
    useCallback,
    useContext,
...
const Actions: {
    ...
    removeArrayValue: (state, { path, index }, omitSavingUndo) => {
        if (isNil(index) || isNil(path)) return state;
    ...
    addArrayValue: (state, { path, value, index }, omitSavingUndo) => {
        if (isNil(path) || isNil(value)) return state;

        const newState = {
            ...state,
        };

        let arr = get(state, path, []) as unknown[];
